...
(function() {
  var FS, Q, _, applyMigration, cassandra, createVersionTable, debugMode, durations, getCassandraClient, getSchemaVersion, listMigrations, logDebug, logError, logInfo, migrate, moduleVersion, moment, parseCassandraHosts, program, quietMode, readConfig, runQuery, runScript;
  ...
  runScript = function() {
    ...
    program.version(moduleVersion).usage('[options] <config_file>').option('-q, --quiet', 'Silence non-error output (default is false)').option('-d, --debug', 'Increase verbosity and error detail').option('-k, --keyspace <keyspace>', 'Cassandra keyspace used for migration and schema_version table').option('-t, --target-version <version>', 'Maximum migration version to apply (default runs all migrations)').option('-h, --hosts <hosts>', 'A comma separated list of cassandra hosts', parseCassandraHosts).parse(process.argv);
