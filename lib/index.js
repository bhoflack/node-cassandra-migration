...
(function() {
  var FS, Q, _, applyMigration, cassandra, cassandrahosts, createVersionTable, debugMode, durations, getCassandraClient, getSchemaVersion, listMigrations, logDebug, logError, logInfo, migrate, moduleVersion, moment, program, quietMode, readConfig, runQuery, runScript;
  ...
  applyMigration = function(config, client, keyspace, file, version) {
    var cql, queries, queryStrings;
    logInfo("Applying migration: " + file);
    queryStrings = _.trim(FS.readFileSync(file, 'utf-8')).split('---');
    cql = ("INSERT INTO " + keyspace + ".schema_version") + " (zero, version, migration_timestamp)" + (" VALUES (0, " + version + ", '" + (moment().toISOString()) + "');");
    queryStrings.push(cql);
    queries = _(queryStrings).map(function(cql) {
      return function() {
        return runQuery(config, client, cql, version);
      };
    }).value();
    return queries.reduce(Q.when, Q(version));
  };

  migrate = function(config, client, keyspace, migrationFiles, schemaVersion) {
    ...
    if (_(migrations).size() > 0) {
      ...
      migrationFunctions = _(migrations).map(function(arg) {
        ...
        file = arg[0], version = arg[1];
        return function() {
          return applyMigration(config, client, keyspace, file, version);
        };
      }).value();
  ...
  cassandrahosts = function(val) {
    return val.split(',');
  };

  runScript = function() {
    var cassandraClient, code, configFile;
    program.version(moduleVersion).usage('[options] <config_file>').option('-q, --quiet', 'Silence non-error output (default is false)').option('-d, --debug', 'Increase verbosity and error detail').option('-k, --keyspace <keyspace>', 'Cassandra keyspace used for migration and schema_version table').option('-t, --target-version <version>', 'Maximum migration version to apply (default runs all migrations)').option('-h, --hosts <hosts>', 'A comma separated list of cassandra hosts').parse(process.argv);
    configFile = _(program.args).last();
    code = 1;
    cassandraClient = void 0;
    return readConfig(configFile).then(function(config) {
      var keyspace, ref, ref1, ref2, ref3, ref4;
      ...
      return Q.all([listMigrations(config), getCassandraClient(config)]).spread(function(migrationFiles, client) {
        ...
        return getSchemaVersion(config, client, keyspace).then(function(schemaVersion) {
          return migrate(config, client, keyspace, migrationFiles, schemaVersion);
